...
- Desync teardown: AEAD nonces derive from the recv counter with no gap tolerance, so a relay-buffer drop (5-min TTL / 50-msg cap) desyncs the stream unrecoverably. Both mirrors treat 2 consecutive post-handshake tag failures as session death: the dApp emits a relay-level `close_channel` (tombstone; an encrypted TERMINATE would be undecipherable) and clears state. The failure counter is scoped strictly to the AEAD open, never to JSON/dispatch/listener errors.
- A paired session is NOT torn down when the wallet app backgrounds/closes (its socket drops within seconds on iOS). `request()` keeps working: it re-joins the channel if needed, lets the relay buffer the encrypted request for the absent wallet, and on mobile deep-links the wallet app awake (`qrlconnect://?wake=<cid>`: query-only on purpose, expo-router maps any host/path to an in-app route and a `resume` path lands on the app's not-found screen) for approval-needing methods except `qrl_requestAccounts` (`walletRedirectOnRequest: false` opts out entirely). `isPaired()` / `isWalletPresent()` expose this state to dApp UIs; only an explicit disconnect or relay tombstone clears the session, which also fails in-flight requests immediately ("Session terminated by wallet") instead of running out the request timeout.
